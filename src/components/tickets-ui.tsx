...
export function TicketAnalytics({ tickets }: { tickets: Ticket[] }) {
...
/** Ticket submission form. Used inline (user view) and inside a dialog (admin view). */
export function NewTicketForm({ onDone, onCancel }: { onDone?: () => void; onCancel?: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        <label className="stamp mb-1.5 block text-muted-foreground">What happened</label>
        <textarea
      ...
      {done && !error && (
        <p className="text-sm text-status-resolved">Ticket filed — IT can see it now.</p>
      )}
...
export function NewTicketDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  return (
    <Dialog open={open} onClose={onClose} title="New ticket">
      <NewTicketForm onDone={onClose} onCancel={onClose} />
    </Dialog>
  );
}
      ...
      <div className="space-y-5">
        ...
        {editing ? (
          <>
            ...
            <div>
              <label className="stamp mb-1.5 block text-muted-foreground">Resolution notes</label>
              <textarea
                className={`${inputClass} min-h-24 resize-y`}
                value={resolution}
          ...
          <>
            {ticket.resolution && (
              <div className="rounded-lg border border-status-resolved/30 bg-status-resolved/5 p-4">
                ...
                <p className="whitespace-pre-wrap text-sm leading-relaxed">{ticket.resolution}</p>
                {ticket.resolved_at && (
